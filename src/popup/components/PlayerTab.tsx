...
const btnClass =
  'py-2 bg-inset hover:bg-lift text-ink rounded-lg border border-line text-xs font-medium min-h-8 flex items-center justify-center gap-1 cursor-pointer focus-visible:ring-2 focus-visible:ring-brand-ring';
    ...
    <div className="space-y-3">
      <div className="p-3 bg-surface rounded-xl border border-line space-y-2.5">
        <div className="flex items-center justify-between">
          <label htmlFor="speed" className="text-xs font-semibold text-ink">
            Oynatma hızı
          </label>
          <span className="font-mono text-xs font-bold text-ink bg-inset px-2 py-0.5 rounded border border-line">
        ...
        <div className="grid grid-cols-7 gap-1">
          {SPEED_PRESETS.map((preset) => (
            <button
              ...
              className={`min-h-8 py-1 text-[11px] font-mono font-medium rounded-md border focus-visible:ring-2 focus-visible:ring-brand-ring ${
                Math.abs(settings.playbackSpeed - preset) < 0.01
                  ? 'bg-ink border-ink text-canvas font-bold'
                  : 'bg-inset border-line text-mute hover:text-ink'
              }`}
            >
              {preset}x
            </button>
      ...
      <div className="p-3 bg-surface rounded-xl border border-line space-y-2">
        <div className="text-xs font-semibold text-ink">Hızlı atlama</div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="flex items-center justify-center gap-2 min-h-10 py-2.5 px-3 bg-surface hover:bg-inset text-ink rounded-xl border border-line focus-visible:ring-2 focus-visible:ring-brand-ring"
        >
          <Tv className="w-3.5 h-3.5 text-mute" aria-hidden />
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 min-h-10 py-2.5 px-3 bg-surface hover:bg-inset text-ink rounded-xl border border-line focus-visible:ring-2 focus-visible:ring-brand-ring"
        >
          <RotateCcw className="w-3.5 h-3.5 text-mute" aria-hidden />
      ...
      <div className="flex items-center justify-between p-3 bg-surface rounded-xl border border-line">
        <div>
          <div className="text-xs font-medium text-ink" id="silence-label">
            Sessizlik atlama
          </div>
          <div className="text-[11px] text-mute">Konuşma duraklarında hızlanır</div>
        </div>
        <label className="relative inline-flex items-center cursor-pointer min-h-8 min-w-8 justify-end">
          ...
          />
          <div className="w-9 h-5 bg-line rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[6px] after:right-[18px] after:bg-ink after:rounded-full after:h-3.5 after:w-3.5 after:transition-all peer-checked:bg-brand peer-focus-visible:ring-2 peer-focus-visible:ring-brand-ring" />
        </label>
      </div>

      <div className="flex items-center justify-between p-3 bg-surface rounded-xl border border-line">
        <div>
          <div className="text-xs font-medium text-ink" id="sc-label">
            Klavye kısayolları
          </div>
          <div className="text-[11px] text-mute">Udemy sayfasında D P J L [ ] S</div>
        </div>
        <label className="relative inline-flex items-center cursor-pointer min-h-8 min-w-8 justify-end">
          <input
            type="checkbox"
      ...
      <div className="p-2.5 bg-canvas rounded-xl border border-line space-y-1.5">
        <div className="text-[11px] font-mono text-mute">Kısayollar</div>
        <div className="grid grid-cols-2 gap-1.5 text-xs">
          {[
            ...
            <div key={label} className="flex items-center justify-between p-1.5 min-h-8 bg-raised rounded-lg border border-line">
              <span className="text-mute">{label}</span>
              <kbd className="px-1.5 py-0.5 bg-inset text-ink rounded font-mono text-[11px]">{keys}</kbd>
            </div>
          ))}
        </div>
      </div>
